...
def speculative_generate(draft_model, teacher_model, tokenizer, prompt, **kwargs):
    ...
    while tokens_generated < MAX_TOKENS: # generate tokens until we reach the max number of tokens
        ...
        for tok_proposed in range(GAMMA): # generate GAMMA tokens from the draft model
            next_token, logits, draft_cache = autoregressive_generate_step(draft_model, input_ids[:, :current_position + tok_proposed], past_key_values=draft_cache)
...
if __name__ == "__main__":
    draft_model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3-0.6B", device_map="auto", torch_dtype=torch.bfloat16)
    teacher_model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3-4B", device_map="auto", torch_dtype=torch.bfloat16)
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-4B", device_map="auto", torch_dtype=torch.bfloat16)
